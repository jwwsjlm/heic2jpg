...
- GUI 桌面版：`heic2jpg-gui-<tag>-windows-amd64.zip`、`heic2jpg-gui-<tag>-macos-universal.tar.gz`、`heic2jpg-gui-<tag>-linux-amd64.tar.gz`
- CLI 命令行版：`heic2jpg-cli-<tag>-windows-amd64.zip`、`heic2jpg-cli-<tag>-linux-amd64.tar.gz`、`heic2jpg-cli-<tag>-darwin-amd64.tar.gz`、`heic2jpg-cli-<tag>-darwin-arm64.tar.gz`
...
```bash
go build -tags cli -ldflags "-X main.version=$(git describe --tags --always --dirty)" -o heic2jpg-cli .
```
...
wails build -ldflags "-X main.version=$(git describe --tags --always --dirty)"
```
