...
一个用 Go + Wails 写的 HEIC/HEIF 批量转 JPG 工具，同时保留命令行版本。GUI 标题栏和首页都会显示当前版本号。

## 主要特点

- 提供桌面 GUI：选择文件/文件夹、调整画质、查看进度和结果更直观，并在窗口标题和界面里显示版本号
...
- GUI 桌面版：`heic2jpg-gui-v0.2.1-windows-amd64.zip`、`heic2jpg-gui-v0.2.1-macos-universal.tar.gz`、`heic2jpg-gui-v0.2.1-linux-amd64.tar.gz`
- CLI 命令行版：`heic2jpg-cli-v0.2.1-windows-amd64.zip`、`heic2jpg-cli-v0.2.1-linux-amd64.tar.gz`、`heic2jpg-cli-v0.2.1-darwin-amd64.tar.gz`、`heic2jpg-cli-v0.2.1-darwin-arm64.tar.gz`
...
```bash
go build -tags cli -ldflags "-X main.version=v0.2.1" -o heic2jpg-cli .
```
...
wails build -ldflags "-X main.version=v0.2.1"
```
